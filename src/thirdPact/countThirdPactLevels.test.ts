...
describe.each(['standard', 'down', 'up'] as const)('roundingMode: %s', (roundingMode) => {
  ...
  it.each([
    [1, 2, 1], [2, 1, 1],
    [1, 5, 2], [3, 3, 2], [5, 1, 2],
    [1, 8, 3], [4, 5, 3], [5, 4, 3], [8, 1, 3],
    [1, 11, 4], [6, 6, 4], [11, 1, 4],
    [1, 14, 5], [7, 8, 5], [8, 7, 5], [14, 1, 5],
    [1, 17, 6], [9, 9, 6], [17, 1, 6],
  ...
  it.each([
    ...
    [3, 20, 20], [6, 19, 20], [9, 18, 20], [20, 20, 20],
  ])('levels third=%i, full=%i returns %i for a multi-class third+full pact caster', (level1, level2, expectedResult) => {
  ...
  it('treats undefined levels as 0', () => {
    const actor = mockActor([
      classes.thirdPact(undefined),
      classes.fullPact(undefined),
      classes.thirdPact(3),
    ]);
...
describe.each(['down', 'standard'] as const)('roundingMode: %s', (roundingMode) => {
  it.each([
    ...
    [1, 4, 1], [2, 3, 1], [3, 2, 1],
    // 7/8
    [1, 6, 2], [2, 5, 2], [4, 3, 2], [3, 4, 2],
    [1, 7, 2], [2, 6, 2], [3, 5, 2], [4, 4, 2],
    // 10/11
    [1, 9, 3], [5, 5, 3],
    [1, 10, 3], [5, 6, 3],
    // 13/14
    ...
    [1, 16, 5], [8, 9, 5],
    // 19/20
    [1, 18, 6], [9, 10, 6], [18, 1, 6],
    [1, 19, 6], [10, 10, 6], [19, 1, 6],
  ])('levels a=%i, b=%i returns %i for a multi-class third pact caster', (level1, level2, expectedResult) => {
    const actor = mockActor([classes.thirdPact(level1), classes.thirdPact(level2)]);
...
describe('roundingMode: up', () => {
  ...
  it.each([
    ...
    [1, 4, 2], [2, 3, 2], [3, 2, 2],
    // 7/8
    [1, 6, 3], [2, 5, 3], [4, 3, 3], [3, 4, 3],
    [1, 7, 3], [2, 6, 3], [3, 5, 3], [4, 4, 3],
    // 10/11
    [1, 9, 4], [5, 5, 4],
    [1, 10, 4], [5, 6, 4],
    // 13/14
    ...
    [1, 16, 6], [8, 9, 6],
    // 19/20
    [1, 18, 7], [9, 10, 7], [18, 1, 7],
    [1, 19, 7], [10, 10, 7], [19, 1, 7],
  ])('levels a=%i, b=%i returns %i for a multi-class third pact caster', (level1, level2, expectedResult) => {
    const actor = mockActor([classes.thirdPact(level1), classes.thirdPact(level2)]);
